...
class RunPodFeature(Feature):
    ...
    def tool_description(self) -> str:
        return (
            "Operator controls for RunPod GPU sessions: start or stop a pod, "
            "inspect status, and view logs"
        )
    ...
    async def generate_image_on_runpod(
        ...
        endpoint = image_status.get("image_endpoint") or image_status.get(
            "inference_url"
        )
        if not endpoint:
        ...
        payload = {
            "prompt": prompt,
            "model": model_name or image_status.get("model_name"),
        }
    ...
    async def _stop(self) -> ToolResult:
        ...
        async with self._start_lock:
            ...
            try:
                ...
                if session_before_stop is not None and self._status_was_active(
                    session_before_stop
                ):
